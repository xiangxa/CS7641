...
def findBestPolicyForForest():
	...
	for size in [1000]:
		...
		# Policy iteration
		policy_iteration = mdp.PolicyIteration(forest.P, forest.R, gamma=0.6, policy0=None, max_iter=1000, eval_type=0)
		...
		# Value iteration
		value_iteration = mdp.ValueIteration(forest.P, forest.R, gamma=0.6, max_iter=1000)
		value_iteration.run()
	...
	plt.xlabel('Iter.', fontsize=12)
	plt.title('Rewards vs Iteration for Forest Mng state 1000 gamma 06', fontsize=12, y=1.03)
	plt.legend()
	plt.savefig('Figures/Forest/Rewards vs Iteration for Forest Mng state 1000 gamma 06.png')
	plt.close()
	...
	for lRate in lRates:
		for epsilon in epsilons:
			...
			q_learning = QLearner.QLearningEx(forest.P, forest.R, grid=np.zeros(shape=(15, 1)), start=0, goals=[14],
											  n_iter=1000, n_restarts=1000, alpha=lRate, gamma=0.6, rar=epsilon,
											  radr=0.999999)
			q_learning.run()
